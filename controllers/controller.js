...
const loadHomepage = async (req, res) => {
  ...
  if (sessionId === undefined) {
    ...
    if (user === null) {
      res.sendFile(
        "/home/batth/sanjha/code/winter23/project/public/login.html",
      );
    } else {
      res.send(`hello ${user}!!`);
    }
...
const register = async (req, res) => {
  ...
  //         if (err) { throw err; }
  //         else {
  //           pool.query('insert into public.users values ($1,$2)', [username, hash], (err, result) => {
  //             if (err) throw err;
  //             else {
  ...
  try {
    ...
    await newuser.save();
    // console.log(newuser.generateAccessToken());
...
const login = async (req, res) => {
  ...
  result = await pool.query("select * from public.users where username=$1", [
    username,
  ]);
  if (result.rows.length == 0) {
    ...
    if (result === true) {
      ...
      await pool.query(
        `update public.users set sessionid = $1 where username = $2`,
        [sessionId, username],
      );

      res.setHeader("set-cookie", [
        `sessionid=${sessionId};httponly;samesite=lax`,
      ]);
...
const validateUser = async (sessionId) => {
  user = await pool.query(
    "select username from public.users where sessionid=$1",
    [sessionId],
  );
  if (user.rowCount === 0) {
    return null;
